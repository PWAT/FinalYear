...
function detectSpecs() {
	...
	if (!hasGetUserMedia) {
		...
		prompt.innerHTML = 'Please allow camera access. Use the mouse to tilt and scroll wheel to zoom. Change parameters with the control panel.  This demo projects webcam video onto a 3D Mesh. It creates a 3D depth map by mapping pixel brightness to Z-depth. Perlin noise is used for the ripple effect and CSS3 filters are used for color effects.';
		init();
	}
...
function init() {
	...
	navigator.getUserMedia({
		...
		prompt.innerHTML = 'Unable to capture WebCam. Please reload the page if you wish to proceed with this demonstration and select ALLOW from the menu at the top of the screen.';
	});
